...
class CompilationCacheTest(CompilationCacheTestCase):
  ...
  def test_diff_executables(self):
    ...
    if jax._src.lib.jaxlib_extension_version < 331:
      executable1 = backend.compile_and_load(computation1, compile_options)
      executable2 = backend.compile_and_load(computation2, compile_options)
    else:
  ...
  def test_put_executable(self):
    ...
    if jax._src.lib.jaxlib_extension_version < 331:
      executable = backend.compile_and_load(str(computation), compile_options)
    else:
